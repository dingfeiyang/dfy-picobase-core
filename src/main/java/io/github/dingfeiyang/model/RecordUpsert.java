...
import io.github.dingfeiyang.file.PbFileSystem;
import io.github.dingfeiyang.interceptor.InterceptorFunc;
import io.github.dingfeiyang.interceptor.Interceptors;
import io.github.dingfeiyang.json.PbJsonTemplate;
import io.github.dingfeiyang.log.PbLog;
import io.github.dingfeiyang.logic.mapper.CollectionMapper;
import io.github.dingfeiyang.logic.mapper.RecordMapper;
import io.github.dingfeiyang.model.schema.SchemaField;
import io.github.dingfeiyang.model.schema.fieldoptions.FileOptions;
import io.github.dingfeiyang.model.validators.RecordDataValidator;
import io.github.dingfeiyang.persistence.resolver.ListUtil;
import io.github.dingfeiyang.persistence.resolver.ResultCouple;
import io.github.dingfeiyang.util.TypeSafe;
...
import static io.github.dingfeiyang.file.PbFileSystem.THUMB_PREFIX;
import static io.github.dingfeiyang.model.validators.Validators.uniqueId;
import static io.github.dingfeiyang.persistence.resolver.ListUtil.subtractList;
import static io.github.dingfeiyang.persistence.resolver.ListUtil.toUniqueStringList;
import static io.github.dingfeiyang.util.PbConstants.*;
import static io.github.dingfeiyang.util.PbConstants.FieldName.*;
import static io.github.dingfeiyang.util.PbConstants.FieldType.Email;
import static io.github.dingfeiyang.util.PbConstants.FieldType.File;
...
public class RecordUpsert implements Validatable {
    ...
    private RuleFunc checkUniqueEmail() {
        return (value) -> {
            ...
            boolean isUnique = recordMapper.isRecordValueUnique(this.record.getCollection().getId(), FieldName.Email, v, this.record.getId());
            if (!isUnique) {
    ...
    public RuleFunc checkUniqueUsername() {
        return (value) -> {
            ...
            boolean isUnique = recordMapper.isRecordValueUnique(this.record.getCollection().getId(), FieldName.Username, v, this.record.getId());
            if (!isUnique) {
    ...
    public void saveRecord() {
        ...
        if (record.getCollection().isAuth()) {
            ...
            List<CollectionModel> authCollections = collectionMapper.findCollectionsByType(CollectionType.Auth);
            ...
            for (CollectionModel collection : authCollections) {
                ...
                boolean isUnique = recordMapper.isRecordValueUnique(collection.getId(), FieldName.Id, record.getId());
                if (!isUnique) {
    ...
    private Errors validateAndFill() {
        ...
        if (this.record.getCollection().isAuth()) {
            ...
            if (!StrUtil.isBlank(this.getUsername())) {
                record.set(FieldName.Username, this.username);
            }

            if (record.isNew() || this.isManageAccess()) {
                record.set(FieldName.Email, this.email);
            }

            record.set(FieldName.EmailVisibility, this.emailVisibility);

            if (this.isManageAccess()) {
                record.set(FieldName.Verified, this.verified);
            }
