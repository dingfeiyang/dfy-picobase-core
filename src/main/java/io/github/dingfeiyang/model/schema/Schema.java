...
public class Schema implements Validatable {
    ...
    public static List<String> fieldValueModifiers() {
        return List.of(PbConstants.FIELD_VALUE_MODIFIER_ADD, PbConstants.FIELD_VALUE_MODIFIER_SUBTRACT);
    }
    ...
    public Err validate() {
         ...
         */
        return Validation.validate(this.fields, Validation.by(value -> {
            ...
            for (int i = 0; i < fields.size(); i++) {
                ...
                if (CollUtil.contains(ids, field.getId())) {
                    return Err.newErrors().put(Integer.toString(i), Err.newErrors().put("id", Err.newError("validation_duplicated_field_id", "Duplicated or invalid schema field id")));
                }
                ...
                if (CollUtil.contains(names, nameLower)) {
                    return Err.newErrors().put(Integer.toString(i), Err.newErrors().put("name", Err.newError("validation_duplicated_field_name", "Duplicated or invalid schema field name")));
                }
