...
public class ThresholdRule implements Rule {
     ...
     */
    private static final Err ErrMinGreaterEqualThanRequired = Err.newError("validation_min_greater_equal_than_required", "must be no less than {{threshold}}");
    /**
     * ErrMaxLessEqualThanRequired is the error that returns when a value is greater than a specified threshold.
     */
    private static final Err ErrMaxLessEqualThanRequired = Err.newError("validation_max_less_equal_than_required", "must be no greater than {{threshold}}");
    /**
     * ErrMinGreaterThanRequired is the error that returns when a value is less than or equal to a specified threshold.
     */
    private static final Err ErrMinGreaterThanRequired = Err.newError("validation_min_greater_than_required", "must be greater than {{threshold}}");
    /**
     * ErrMaxLessThanRequired is the error that returns when a value is greater than or equal to a specified threshold.
     */
    private static final Err ErrMaxLessThanRequired = Err.newError("validation_max_less_than_required", "must be less than {{threshold}}");
    ...
    public Err validate(Object value) {
        ...
        switch (threshold.getClass().getSimpleName()) {
            ...
            default -> {
                return Err.newError("", "Type not supported: " + threshold.getClass().getSimpleName());
            }
        }
