...
/**
 * 在调用 `PbUtil.login()` 时的 配置参数 Model，决定登录的一些细节行为 <br>
 *
 * <pre>
 *     	// 例如：在登录时指定 token 有效期为七天，代码如下：
 *     	PbUtil.login(10001, new PbLoginModel().setTimeout(60 * 60 * 24 * 7));
 * </pre>
 */
public class PbLoginModel {
     ...
     * @return 对象自身
     */
    public PbLoginModel setDevice(String device) {
        this.device = device;
        return this;
    }
     ...
     * @return 对象自身
     */
    public PbLoginModel setIsLastingCookie(Boolean isLastingCookie) {
     ...
     * @return 对象自身
     */
    public PbLoginModel setTimeout(long timeout) {
     ...
     * @return 对象自身
     */
    public PbLoginModel setActiveTimeout(long activeTimeout) {
     ...
     * @return 对象自身
     */
    public PbLoginModel setExtraData(Map<String, Object> extraData) {
     ...
     * @return 对象自身
     */
    public PbLoginModel setToken(String token) {
        this.token = token;
        return this;
    }
     ...
     * @return 对象自身
     */
    public PbLoginModel setIsWriteHeader(Boolean isWriteHeader) {
     ...
     * @return 对象自身
     */
    public PbLoginModel setTokenSignTag(Object tokenSignTag) {
    ...
    public Long getTimeoutOrGlobalConfig() {
        if (timeout == null) {
            timeout = PbManager.getConfig().getTimeout();
        }
        return timeout;
    }
    ...
    public Boolean getIsWriteHeaderOrGlobalConfig() {
        if (isWriteHeader == null) {
            isWriteHeader = PbManager.getConfig().getIsWriteHeader();
        }
        return isWriteHeader;
    }
     ...
     * @return 对象自身
     */
    public PbLoginModel setExtra(String key, Object value) {
    ...
    public int getCookieTimeout() {
        ...
        if (getTimeoutOrGlobalConfig() == PbCache.NEVER_EXPIRE) {
    ...
    public String getDeviceOrDefault() {
        if (device == null) {
            return PbConstants.DEFAULT_LOGIN_DEVICE;
        }
        return device;
    }
     ...
     * @return 对象自身
     */
    public PbLoginModel build() {
        return build(PbManager.getConfig());
    }
     ...
     * @return 对象自身
     */
    public PbLoginModel build(PbConfig config) {
        if (timeout == null) {
