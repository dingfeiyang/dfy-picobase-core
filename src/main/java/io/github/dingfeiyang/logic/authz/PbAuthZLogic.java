package io.github.dingfeiyang.logic.authz;

import cn.hutool.core.util.ClassUtil;
import io.github.dingfeiyang.PbManager;
import io.github.dingfeiyang.cache.PbCache;
import io.github.dingfeiyang.config.PbConfig;
import io.github.dingfeiyang.config.PbCookieConfig;
import io.github.dingfeiyang.context.PbHolder;
import io.github.dingfeiyang.context.model.PbCookie;
...
import io.github.dingfeiyang.error.PbErrorCode;
import io.github.dingfeiyang.fun.PbFunction;
import io.github.dingfeiyang.listener.PbEventCenter;
import io.github.dingfeiyang.session.PbSession;
import io.github.dingfeiyang.session.TokenSign;
import io.github.dingfeiyang.strategy.PbStrategy;
import io.github.dingfeiyang.util.CommonHelper;
import io.github.dingfeiyang.util.PbConstants;
import io.github.dingfeiyang.util.PbValue2Box;
import io.github.dingfeiyang.annotation.*;
import io.github.dingfeiyang.exception.*;
...
public class PbAuthZLogic {
        ...
        if (CommonHelper.isNotEmpty(tokenPrefix)) {
            ...
            // 情况2.2：如果 token 有值，但是并不是以指定的前缀开头
                ...
                if (noPrefixThrowException) {
                    throw NotLoginException.newInstance(loginType, NotLoginException.NO_PREFIX, NotLoginException.NO_PREFIX_MESSAGE + "，prefix=" + tokenPrefix, null).setCode(PbErrorCode.CODE_11017);
                } else {
                    tokenValue = null;
                }
            }
    ...
    public String getTokenValueNotNull() {
        ...
        if (CommonHelper.isEmpty(tokenValue)) {
            throw NotLoginException.newInstance(loginType, NotLoginException.NOT_TOKEN, NotLoginException.NOT_TOKEN_MESSAGE, null).setCode(PbErrorCode.CODE_11001);
        }
        return tokenValue;
    }
    ...
    public Object getLoginId() {
        ...
        String tokenValue = getTokenValue(true);
        if (CommonHelper.isEmpty(tokenValue)) {
            throw NotLoginException.newInstance(loginType, NotLoginException.NOT_TOKEN, NotLoginException.NOT_TOKEN_MESSAGE, null).setCode(PbErrorCode.CODE_11011);
        }

        // 3、查找此 token 对应的 loginId，如果找不到则抛出：token 无效
        String loginId = getLoginIdNotHandle(tokenValue);
        if (CommonHelper.isEmpty(loginId)) {
            throw NotLoginException.newInstance(loginType, NotLoginException.INVALID_TOKEN, NotLoginException.INVALID_TOKEN_MESSAGE, tokenValue).setCode(PbErrorCode.CODE_11012);
        }

        // 4、如果这个 token 指向的是值是：过期标记，则抛出：token 已过期
        if (loginId.equals(NotLoginException.TOKEN_TIMEOUT)) {
            throw NotLoginException.newInstance(loginType, NotLoginException.TOKEN_TIMEOUT, NotLoginException.TOKEN_TIMEOUT_MESSAGE, tokenValue).setCode(PbErrorCode.CODE_11013);
        }

        // 5、如果这个 token 指向的是值是：被顶替标记，则抛出：token 已被顶下线
        if (loginId.equals(NotLoginException.BE_REPLACED)) {
            throw NotLoginException.newInstance(loginType, NotLoginException.BE_REPLACED, NotLoginException.BE_REPLACED_MESSAGE, tokenValue).setCode(PbErrorCode.CODE_11014);
        }

        // 6、如果这个 token 指向的是值是：被踢下线标记，则抛出：token 已被踢下线
        if (loginId.equals(NotLoginException.KICK_OUT)) {
            throw NotLoginException.newInstance(loginType, NotLoginException.KICK_OUT, NotLoginException.KICK_OUT_MESSAGE, tokenValue).setCode(PbErrorCode.CODE_11015);
        }
    ...
    public void checkActiveTimeout(String tokenValue) {
        ...
        storage.get(PbConstants.TOKEN_ACTIVE_TIMEOUT_CHECKED_KEY, () -> {
            ...
            if (activeTimeout == PbCache.NOT_VALUE_EXPIRE) {
                throw NotLoginException.newInstance(loginType, NotLoginException.TOKEN_FREEZE, NotLoginException.TOKEN_FREEZE_MESSAGE, tokenValue).setCode(PbErrorCode.CODE_11016);
            }
