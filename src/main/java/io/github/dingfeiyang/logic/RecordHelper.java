...
public class RecordHelper {
    ...
    private static String[] ruleQueryParams = {PbConstants.QueryParam.FILTER, PbConstants.QueryParam.SORT};
    ...
    public static RequestInfo createRequestInfo() {
        ...
        RequestInfo requestInfo = (RequestInfo) PbHolder.getStorage().get(PbConstants.REQUEST_INFO_KEY);
        ...
        while (headerNames.hasMoreElements()) {
            ...
            if (StrUtil.isNotEmpty(v)) {
                requestInfo.getHeaders().put(DbxUtil.snakeCase(k), v);
            }
        }
        ...
        PbHolder.getStorage().set(PbConstants.REQUEST_INFO_KEY, requestInfo);
    ...
    private static void setAuthInfo(RequestInfo requestInfo) {
        ...
        if (StrUtil.isNotEmpty(authRecordId)) { // 增加authRecordId检查， 防止getExtra 时，loginType检查异常
            String collectionId = (String) PbUtil.getExtra(PbConstants.JwtExtraFieldCollectionId);
    ...
    public static Error enrichRecords(List<RecordModel> records, String... defaultExpands) {
        ...
        var param = PbHolder.getRequest().getParameter(PbConstants.QueryParam.EXPAND);
    ...
    private static void autoIgnoreAuthRecordsEmailVisibility(List<RecordModel> records, RequestInfo requestInfo) {
        ...
        SelectQuery query = recordMapper.recordQuery(collection)
                .select(DbxUtil.quoteSimpleColumnName(collection.getName()) + ".id")
                .andWhere(Expression.in(DbxUtil.quoteSimpleColumnName(collection.getName()) + ".id", recordIds));
