...
import io.github.dingfeiyang.persistence.resolver.ResultCouple;
...
import io.github.dingfeiyang.persistence.resolver.DbUtil;
...
public class RecordMapper extends AbstractMapper<RecordModel> {
    ...
    public Optional<RecordModel> findRecordById(String collectionNameOrId, String recordId, Consumer<SelectQuery>... optFilters) {
        ...
        SelectQuery query = recordQuery.andWhere(Expression.newHashExpr(Map.of(collection.getName() + ".id", recordId)));
    ...
    private Error expand(List<RecordModel> records, String expandPath, ExpandFetchFunc fetchFunc, int recursionLevel) {
        ...
        Matcher matcher = PbConstants.IndirectExpandRegexPattern.matcher(parts[0]);
        if (matcher.matches() && matcher.groupCount() == 2) {
            ...
            if (indirectRelField == null || !indirectRelField.getType().equals(PbConstants.FieldType.Relation)) {
            ...
            Supplier<Error> prepErr = () -> {
                try {
                    ...
                    if (indirectRelFieldOptions.isMultiple()) {
                        q.innerJoin(
                                String.format("( SELECT `id` FROM %s,%s `je` WHERE `je`.`value` = :id ) je2", indirectRel.getName(), DbUtil.jsonEach(indirectRelField.getName()))
                                , Expression.newExpr(String.format("`je2`.`id` = `%s`.`id`", indirectRel.getName())));
                        ...
                        q.andWhere(Expression.newExpr(DbxUtil.quoteColumnName(indirectRelField.getName()) + " = :id"));
                    }
            ...
            if (DbUtil.hasSingleColumnUniqueIndex(indirectRelField.getName(), indirectRel.getIndexes())) {
            ...
            relField = new SchemaField()
                    ...
                    .setType(PbConstants.FieldType.Relation)
                    .setName(parts[0])
            ...
            if (relField == null || !relField.getType().equals(PbConstants.FieldType.Relation)) {
    ...
    public SelectQuery recordQuery(CollectionModel collection) {
        ...
        var selectCols = String.format("%s.*", DbxUtil.quoteSimpleColumnName(tableName));
    ...
    public List<RecordModel> findRecordByIds(String collectionNameOrId, List<String> relIds, Consumer<SelectQuery>... optFilters) {
        ...
        var query = this.recordQuery(collection.get())
                .andWhere(Expression.in(collection.get().getName() + ".id", ListUtil.toUniqueStringList(relIds)));
    ...
    public boolean isRecordValueUnique(String collectionNameOrId, String key, Object value, String... excludeIds) {
        ...
        if (collection.isAuth() && PbConstants.FieldName.Username.equals(key)) {
            ...
            expr = Expression.newHashExpr(Map.of(DbxUtil.columnify(key), normalizedVal));
        }
    ...
    public void updateRecord(RecordModel model) {
         ...
         */
        PbUtil.getPbDbxBuilder().update(model.tableName(), BeanUtil.beanToMap(model.columnValueMap()), Expression.newHashExpr(Map.of("id", model.getId()))).execute();
    ...
    public void cascadeRecordDelete(RecordModel mainRecord, Map<CollectionModel, List<SchemaField>> refs) {
        ...
        for (CollectionModel refCollection : sortedRefKeys) {
            ...
            for (SchemaField field : fields) {
                String recordTableName = DbxUtil.columnify(refCollection.getName());
                String prefixedFieldName = recordTableName + "." + DbxUtil.columnify(field.getName());
                ...
                while (true) {
                    ...
                    List<RecordModel> refRecords = RecordModel.newRecordsFromNullStringMaps(refCollection, originalDatas);
    ...
    public RecordModel findRecordByViewFile(CollectionModel view, String fileFieldName, String filename) {
        ...
        String cleanFieldName = DbxUtil.columnify(qf.getOriginal().getName());
        ...
        if (!(qf.getOriginal().getOptions() instanceof MultiValuer options) || !options.isMultiple()) {
            query.andWhere(Expression.newHashExpr(Map.of(cleanFieldName, filename)));
        } else {
            //TODO
    ...
    public RecordModel findAuthRecordByEmail(CollectionModel collection, String identity) {
        ...
        SelectQuery recordQuery = this.recordQuery(collection);
        SelectQuery query = recordQuery.andWhere(Expression.newHashExpr(Map.of(PbConstants.FieldName.Email, identity)));
    ...
    public RecordModel findAuthRecordByUsername(CollectionModel collection, String identity) {
        ...
        SelectQuery recordQuery = this.recordQuery(collection);
        SelectQuery query = recordQuery.andWhere(Expression.newHashExpr(Map.of(PbConstants.FieldName.Username, identity)));
        return query.limit(1).one(new RecordRowMapper(collection));
    }
}
