...
import io.github.dingfeiyang.cache.LazyCache;
import io.github.dingfeiyang.exception.BadRequestException;
import io.github.dingfeiyang.exception.PbException;
import io.github.dingfeiyang.model.schema.MultiValuer;
import io.github.dingfeiyang.model.schema.Schema;
import io.github.dingfeiyang.model.schema.SchemaField;
import io.github.dingfeiyang.model.schema.fieldoptions.JsonOptions;
...
import io.github.dingfeiyang.persistence.mapper.MappingOptions;
import io.github.dingfeiyang.persistence.model.Index;
import io.github.dingfeiyang.persistence.repository.StorageContextHolder;
import io.github.dingfeiyang.util.PbConstants;
import io.github.dingfeiyang.util.Tokenizer;
import io.github.dingfeiyang.model.*;
import io.github.dingfeiyang.persistence.dbx.DbxUtil;
import io.github.dingfeiyang.persistence.resolver.ListUtil;
...
public class CollectionMapper extends AbstractMapper<CollectionModel> {
    ...
    public boolean isCollectionNameUnique(String name, String... ids) {
        SelectQuery sq = super.modelQuery().select("count(*)").andWhere(Expression.newExpr("LOWER(`name`)=:name", Map.of("name", name.toLowerCase()))).limit(1);
        ids = ArrayUtil.removeNull(ids);
        ids = ArrayUtil.distinct(ids);

        if (ids.length > 0) {
            sq.andWhere(Expression.notIn("id", (Object[]) ids));
        }
    ...
    public CollectionModel findCollectionByNameOrId(String nameOrId) {
        ...
        return cache.get(nameOrId, () -> modelQuery()
                .andWhere(Expression.newExpr("`id` = :id OR LOWER(`name`)=:name"
    ...
    public Schema createViewSchema(String selectQuery) {
        ...
        try {
            ...
            for (TableInfoRow row : info) {
                if (PbConstants.FieldName.Id.equals(row.getName())) {
                    hasId = true;
                }

                if (ArrayUtil.contains(PbConstants.baseModelFieldNames, row.getName())) {
    ...
    public Map<String, QueryField> parseQueryToFields(String selectQuery) {
        ...
        for (Identifier col : p.getColumns()) {
            ...
            if (colLower.startsWith("count(") || colLower.startsWith("total(")) {
                ...
                queryField.setField(new SchemaField(col.getAlias(), PbConstants.FieldType.Number));
            ...
            if (castMatcher.find() && castMatcher.groupCount() == 1) {
                ...
                switch (castMatch) {
                    ...
                    case "text" -> {
                        queryField.setField(new SchemaField(col.getAlias(), PbConstants.FieldType.Text));
                        result.put(col.getAlias(), queryField);
                        continue;
                    }
                    case "boolean", "bool" -> {
            ...
            if (fieldName.equals(PbConstants.FieldName.Id)) {
                // convert to relation since it is a direct id reference
                QueryField queryField = new QueryField();
                queryField.setField(new SchemaField(col.getAlias(), PbConstants.FieldType.Relation, new RelationOptions(1, collection.getId())));
                queryField.setCollection(collection);
                result.put(col.getAlias(), queryField);
            } else if (fieldName.equals(PbConstants.FieldName.Created) || fieldName.equals(PbConstants.FieldName.Updated)) {
                QueryField queryField = new QueryField();
                queryField.setField(new SchemaField(col.getAlias(), PbConstants.FieldType.Date));
                queryField.setCollection(collection);
                result.put(col.getAlias(), queryField);
            } else if (fieldName.equals(PbConstants.FieldName.Username) && collection.isAuth()) {
                QueryField queryField = new QueryField();
                queryField.setField(new SchemaField(col.getAlias(), PbConstants.FieldType.Text));
                queryField.setCollection(collection);
                result.put(col.getAlias(), queryField);
                ...
                queryField.setCollection(collection);
                result.put(col.getAlias(), queryField);
            } else if ((fieldName.equals(PbConstants.FieldName.Verified) || fieldName.equals(PbConstants.FieldName.EmailVisibility)) && collection.isAuth()) {
                QueryField queryField = new QueryField();
                queryField.setField(new SchemaField(col.getAlias(), PbConstants.FieldType.Bool));
                queryField.setCollection(collection);
                result.put(col.getAlias(), queryField);
    ...
    private List<CollectionModel> selectList(String type, List<String> names, List<String> excludeIds) {
        ...
        if (StrUtil.isNotEmpty(type)) {
            selectQuery.andWhere(Expression.newExpr("`type` = :type", Map.of("type", type)));
        }

        if (CollUtil.isNotEmpty(names)) {
            selectQuery.andWhere(Expression.in("`name`", names));
        }

        if (CollUtil.isNotEmpty(excludeIds)) {
            selectQuery.andWhere(Expression.notIn("`id`", excludeIds));
        }
    ...
    private String normalizeViewQueryId(String query) {
        ...
        QueryField idField = parsed.get(PbConstants.FieldName.Id);
        if (idField != null && idField.getField() != null &&
                !Objects.equals(idField.getField().getType(), PbConstants.FieldType.Json) &&
                !Objects.equals(idField.getField().getType(), PbConstants.FieldType.Number) &&
                !Objects.equals(idField.getField().getType(), PbConstants.FieldType.Bool)) {
            needWrapping = false;
        }
    ...
    public void syncRecordTableSchema(CollectionModel newCollection, CollectionModel oldCollection) {
        ...
        if (null == oldCollection) {
            ...
            cols.put(PbConstants.FieldName.Id, "VARCHAR(32) PRIMARY KEY NOT NULL");
            cols.put(PbConstants.FieldName.Created, "DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL");
            cols.put(PbConstants.FieldName.Updated, "DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL");

            if (newCollection.isAuth()) {
                cols.put(PbConstants.FieldName.Username, "VARCHAR(255) NOT NULL");
                cols.put(PbConstants.FieldName.Email, "VARCHAR(255) NOT NULL");
                cols.put(PbConstants.FieldName.EmailVisibility, "BOOLEAN DEFAULT FALSE NOT NULL");
                cols.put(PbConstants.FieldName.Verified, "BOOLEAN DEFAULT FALSE NOT NULL");
                cols.put(PbConstants.FieldName.TokenKey, "VARCHAR(255) NOT NULL");
                cols.put(PbConstants.FieldName.PasswordHash, "TEXT NOT NULL");
                cols.put(PbConstants.FieldName.LastResetSentAt, "DATETIME DEFAULT NULL");
                cols.put(PbConstants.FieldName.LastVerificationSentAt, "DATETIME DEFAULT NULL");
            }
            ...
            String tableSchema = cols.entrySet()
                    .stream()
                    .map(entry -> DbxUtil.quoteSimpleColumnName(entry.getKey()) + " " + entry.getValue())
    ...
    public void resaveViewsWithChangedSchema(String... excludeIds) {
        List<CollectionModel> collections = findCollectionsByType(PbConstants.CollectionType.View);

        for (CollectionModel collection : collections) {
            if (excludeIds.length > 0 && ListUtil.existInArray(collection.getId(), excludeIds)) {
                continue;
            }
    ...
    public Map<CollectionModel, List<SchemaField>> findCollectionReferences(CollectionModel collection, String... excludeIds) {
        ...
        for (CollectionModel c : collections) {
            for (SchemaField f : c.getSchema().getFields()) {
                if (!StrUtil.equals(f.getType(), PbConstants.FieldType.Relation)) {
                    continue;
                }
                f.initOptions();
