...
import static io.github.dingfeiyang.persistence.model.Index.parseIndex;
...
public class DbUtil {
    public static String jsonArrayLength(String column) {
        return String.format("JSON_LENGTH(IF(JSON_VALID(%s),%s,IF(%s='',JSON_ARRAY(),JSON_ARRAY(%s))))",
                DbxUtil.quoteColumnName(column), DbxUtil.quoteColumnName(column), DbxUtil.quoteColumnName(column), DbxUtil.quoteColumnName(column));
    }
    ...
    public static String jsonEach(String column) {
        ...
        return String.format("JSON_TABLE( IF(JSON_VALID(%s), %s, JSON_ARRAY(%s)), '$[*]' COLUMNS ( `value` VARCHAR(255) PATH '$' ) )"
                , DbxUtil.quoteColumnName(column), DbxUtil.quoteColumnName(column), DbxUtil.quoteColumnName(column));
